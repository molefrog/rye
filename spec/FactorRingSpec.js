...
describe('class FactorRing', function() {
	...
	it('has order property', function() {
		f1.order.should.eql(3*3);
		f2.order.should.eql(1);
	});
	
	it('has null element', function() {
		var i=0;
		f1.nullElement().should.eql(0);
		for(i=0; i<f1.order; ++i) {
			f1.add(i, f1.nullElement()).should.eql(i);
		}
	});
	
	it('has one element if base is irreducible', function() {
		var i=0;
		f1.oneElement().should.eql(1);
		
		for(i=0; i<f1.order; ++i) {
			f1.mul(i, f1.oneElement()).should.eql(i);
		}
	});	
	...
	it('can add two elements', function() {
		...
		).should.eql(f1.index(ring3.polynom([1, 1])));	
		
		f1.add( 
			f1.index( ring3.polynom([2, 2])), 
			f1.index( ring3.polynom([1, 2]))
		).should.eql(f1.index(ring3.polynom([0, 1])));	
	});
	...
	it('can multiply two element', function() {
		f1.mul( 
			f1.index( ring3.polynom([0, 2])), 
			f1.index( ring3.polynom([2, 2]))
		).should.eql(f1.index(ring3.polynom([2, 1])));	
	});
	...
	it('can obtain opposite element', function() {
		...
		for(i=0; i<f1.order; ++i) {
			f1.add(i, f1.opp(i)).should.eql(f1.nullElement());
		}
	});
	...
	it('can obtain inverse element', function() {
		...
		for(i=0; i<f1.order; ++i) {
			var inv = f1.inv(i);
			if(!isNaN(inv)) {
				f1.mul(i, inv).should.eql(f1.oneElement());
			}
		}
	});
