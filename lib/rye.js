...
function PolynomRing(field) {
	...
	this.polynom = function (coefficients) {
        ...
        while (_power >= 0 && coefficients[_power] === this.field.nullElement()) {
            --_power;
        }
	...
	this.factorSet = function(element) {
		...
		if(element.power() === -Infinity) {
			throw 'Factorization by null element';
		}
		
		// Calculating the power of the factor set
		var order = Math.pow(this.field.order, element.power());
	...
	this.toLatex = function(f, options) {
		...
		var i = 0;
		for(i = 0; i <= f.power(); ++i) {
			// Will not draw monoms with null coefficient
			if(f.coef(i) !== this.field.nullElement() ) {
				var currentPower = '';
				
				if(i === 0) {
					// Not draw x at 0 degree
					currentPower = '';
				}
				else if(i === 1) {
					// Draw only x at 1 degree
					currentPower = 'x';
				} else {
					// Draw powers of x at other degrees
					currentPower = 'x^{'+i + '}';
				}	
